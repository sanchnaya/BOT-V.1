exports.private = () =>{ return`Feature can only be used in private chat` } exports.wait = () => { return `⏳ Please wait...` } exports.ok = () => { return `✅ Done.` } exports.err = () => { return `⚠️ Error` } exports.erorLink = () => { return `⚠️ Link error` } exports.media = () => { return `Media Downloading...` } exports.replyImg = (prefix, command) => { return `Send/Reply Photo With Caption ${prefix + command}` } exports.wrongFormat = (prefix) => { return `Incorrect format Please check how to use in *${prefix}allmenu*.` } exports.emptyMess = () => { return `❎ Please enter the message you want to convey!` } exports.cmdNotFound = (cmd, prefix) => { return `❎ Command *${cmd}* not listed in *${prefix}allmenu*` } exports.ownerOnly = () => { return `❎ Command Owner only` } exports.doneOwner = () => { return `✅ ️It's done, Owner ~` } exports.groupOnly = () => { return `👥 This command can only be used within groups!` } exports.adminOnly = () => { return `🙅 Command Only Admin` } exports.nhFalse = () => { return `Invalid code!` } exports.listBlock = (blockNumber) => { return `*── HALL OF SHAME * Total blocked: *${blockNumber.length}* user\n` } exports.notAdmin = () => { return `❎ User is not an admin! ` } exports.adminAlready = () => { return `❎ Cannot promote user who is admin!` } exports.botNotAdmin = () => { return `Make bot admin first! 🙏` } exports.received = (pushname) => { return ` Hello ${pushname}! Thank you for reporting, we will receive your report soon.` } exports.videoLimit = () => { return `The file size is too big!` } exports.notNum = (q) => { return `"${q}", not a number!` } exports.menu = (greetings, pushname, botname) =>{ return`Hi User ${pushname} powered By BOT V.1 , Beta Multi-Device Whatsapp Bot 24 hour power on bot.... Sport replay with cmd.... ` } exports.listMenu = (time, greeting, pushname, prefix) => { return `*Congratulations ${greeting} ${pushname}* *⌚Time Server : ${time}* *📚 List-Menu BOT V.1 :*
